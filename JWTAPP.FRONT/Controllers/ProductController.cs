    ...
    public class ProductController : Controller
    {
        ...
        {
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.GetAsync("http://localhost:5110/api/products");
        ...
        public async Task<IActionResult> Remove (int id)
        {
            ...
            if (token != null)
            {
                var client= _httpClientFactory.CreateClient();
                client.DefaultRequestHeaders.Authorization= new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                var response = await client.DeleteAsync($"http://localhost:5110/api/products/{id}");
            }
        ...
        {
            var cateData = TempData["Categories"]?.ToString();
            if (cateData != null)
            {
                var categories = JsonSerializer.Deserialize<List<SelectListItem>>(cateData);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.PostAsync("http://localhost:5110/api/products", content);
                    if(response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
        ...
        public async Task <IActionResult> Update(int id)
        {
            ...
            if (token != null)
            {
                var client = _httpClientFactory.CreateClient();
                client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                var responseProduct = await client.GetAsync($"http://localhost:5110/api/products/{id}");
                var responseCategory = await client.GetAsync($"http://localhost:5110/api/category");
        ...
        public async Task<IActionResult> Update(UpdateProductModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var jsonData = JsonSerializer.Serialize(model);
                    var content = new StringContent(jsonData, Encoding.UTF8, "application/json");
                    var response = await client.PutAsync("http://localhost:5110/api/products", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
